...
@onready var center_x_spin = $VBoxContainer/ThreeByThreeContainer/PlacementContainer/CenterX/SpinBox
@onready var center_z_spin = $VBoxContainer/ThreeByThreeContainer/PlacementContainer/CenterZ/SpinBox
@onready var place_pattern_button = $VBoxContainer/ThreeByThreeContainer/PlacementContainer/PlaceButton
@onready var create_button = $VBoxContainer/ThreeByThreeContainer/PlacementContainer/ButtonContainer/CreateButton
@onready var clear_button_3x3 = $VBoxContainer/ThreeByThreeContainer/PlacementContainer/ButtonContainer/ClearButton
@onready var detect_button = $VBoxContainer/ThreeByThreeContainer/PlacementContainer/ButtonContainer/DetectButton
@onready var info_label = $VBoxContainer/ThreeByThreeContainer/InfoLabel
...
func connect_signals():
	...
	place_pattern_button.pressed.connect(_on_place_pattern_pressed)
	center_x_spin.value_changed.connect(_on_center_changed)
	center_z_spin.value_changed.connect(_on_center_changed)
	create_button.pressed.connect(_on_create_3x3_pressed)
	clear_button_3x3.pressed.connect(_on_clear_3x3_pressed)
	detect_button.pressed.connect(_on_detect_3x3_pressed)
...
func add_item_state_ui(item_state: CustomItemState):
	...
	if enhanced_gridmap and enhanced_gridmap.mesh_library:
		...
		item_selector.add_item("No MeshLibrary", -1)
		item_selector.disabled = true
...
func _check_mesh_library():
	if not enhanced_gridmap or not enhanced_gridmap.mesh_library:
		info_label.text = "3x3 Status: No MeshLibrary assigned!"
		pattern_selector.disabled = true
		add_pattern_button.disabled = true
		place_pattern_button.disabled = true
		create_button.disabled = true
		clear_button_3x3.disabled = true
		detect_button.disabled = true
	else:
		info_label.text = "3x3 Status: Ready"
		pattern_selector.disabled = false
		add_pattern_button.disabled = false
		place_pattern_button.disabled = false
		create_button.disabled = false
		clear_button_3x3.disabled = false
...
func _update_fill_options():
	...
	if enhanced_gridmap and enhanced_gridmap.mesh_library:
		...
		fill_options.add_item("No MeshLibrary", -1)
		fill_options.disabled = true
...
func _update_grid_ui():
	...
	if not enhanced_gridmap or not enhanced_gridmap.mesh_library:
		info_label.text = "3x3 Status: No EnhancedGridMap or MeshLibrary"
		return
	...
	for z in range(enhanced_gridmap.rows):
		...
		for x in range(enhanced_gridmap.columns):
			...
			if cell_item != -1:
				for j in range(1, option.get_item_count()):
					if option.get_item_id(j) == cell_item:
						selected_index = j
						break
			...
			var current_rotation = enhanced_gridmap.get_cell_item_orientation(Vector3i(x, current_floor, z))
...
func _on_randomize_pressed():
	if enhanced_gridmap:
		...
		if randomize_states.is_empty():
			info_label.text = "3x3 Status: No states selected for randomization"
			return

		if total_percentage != 100:
			info_label.text = "3x3 Status: Warning: Total randomize percentage is not 100%"
...
func _on_fill_pressed():
	if enhanced_gridmap:
		...
		if selected_index >= 0:
			...
			info_label.text = "3x3 Status: Grid filled with item " + str(selected_index)
		else:
			info_label.text = "3x3 Status: No item selected for filling"
...
func _on_cell_item_selected(index: int, option: OptionButton):
	...
	if enhanced_gridmap:
		...
		info_label.text = "3x3 Status: Cell updated at " + str(position)
...
func _on_find_path_pressed():
	if enhanced_gridmap:
		...
		if path.is_empty():
			...
			info_label.text = "3x3 Status: No path found"
		else:
			path_result_label.text = "Path found: " + str(path)
			info_label.text = "3x3 Status: Path found with " + str(path.size()) + " steps"

func _on_diagonal_movement_toggled(button_pressed):
	if enhanced_gridmap:
		enhanced_gridmap.set_diagonal_movement(button_pressed)
		info_label.text = "3x3 Status: Diagonal movement " + ("enabled" if button_pressed else "disabled")

func _on_normal_item_changed(value):
	if enhanced_gridmap:
		enhanced_gridmap.normal_items[0] = value
		info_label.text = "3x3 Status: Normal item set to " + str(value)

func _on_hover_item_changed(value):
	if enhanced_gridmap:
		enhanced_gridmap.hover_item = value
		info_label.text = "3x3 Status: Hover item set to " + str(value)

func _on_start_item_changed(value):
	if enhanced_gridmap:
		enhanced_gridmap.start_item = value
		info_label.text = "3x3 Status: Start item set to " + str(value)

func _on_end_item_changed(value):
	if enhanced_gridmap:
		enhanced_gridmap.end_item = value
		info_label.text = "3x3 Status: End item set to " + str(value)

func _on_non_walkable_item_changed(value):
	if enhanced_gridmap:
		enhanced_gridmap.non_walkable_items[0] = value
		info_label.text = "3x3 Status: Non-walkable item set to " + str(value)

func _on_grid_updated():
	update_ui()
	enhanced_gridmap._update_cell_option_buttons()
	info_label.text = "3x3 Status: Grid updated"
...
func _on_cell_rotation_changed(index: int, option: OptionButton):
	...
	if enhanced_gridmap:
		if index >= 0 and index < 4:
			...
			info_label.text = "3x3 Status: Cell rotation updated at " + str(position)
...
func _on_swap_items_pressed():
	if enhanced_gridmap:
		...
		info_label.text = "3x3 Status: Swapped item " + str(old_item) + " with " + str(new_item)

func _on_3x3_mode_toggled(button_pressed):
	if enhanced_gridmap:
		enhanced_gridmap.is_3x3_mode = button_pressed
		enhanced_gridmap.initialize_astar()
		enhanced_gridmap.detect_existing_3x3_structures(floor_spin.value)
		_update_3x3_ui()
		info_label.text = "3x3 Status: 3x3 mode " + ("enabled" if button_pressed else "disabled")
...
func _on_pattern_selected(index):
	if enhanced_gridmap and pattern_selector.get_item_count() > 0:
		...
		info_label.text = "3x3 Status: Editing pattern " + pattern_name

func _on_add_pattern_pressed():
	if enhanced_gridmap:
		_show_add_pattern_dialog()
		info_label.text = "3x3 Status: Adding new pattern"

func _update_3x3_ui():
	if not enhanced_gridmap:
		info_label.text = "3x3 Status: No EnhancedGridMap set"
		return
	
	three_by_three_mode_check.button_pressed = enhanced_gridmap.is_3x3_mode
	_populate_pattern_selector()
	info_label.text = "3x3 Status: Updated 3x3 UI"

func _populate_pattern_selector():
	if not enhanced_gridmap:
		info_label.text = "3x3 Status: No EnhancedGridMap"
		return
	...
	if enhanced_gridmap.three_by_three_patterns.is_empty() and not enhanced_gridmap.three_by_three_items.is_empty():
		var default_pattern = enhanced_gridmap.three_by_three_items.duplicate()
		_validate_pattern_size(default_pattern)
		enhanced_gridmap.three_by_three_patterns["default"] = default_pattern
		pattern_selector.add_item("default")
	
	if pattern_selector.get_item_count() > 0:
		pattern_selector.select(0)
		_display_pattern_editor(pattern_selector.get_item_text(0))
		info_label.text = "3x3 Status: Patterns loaded"

func _validate_pattern_size(pattern: Array):
	...
	if enhanced_gridmap and enhanced_gridmap.mesh_library:
		var item_list = enhanced_gridmap.mesh_library.get_item_list()
		var max_id = item_list[item_list.size() - 1] if not item_list.is_empty() else 0
		for i in range(pattern.size()):
			pattern[i] = clamp(pattern[i], -1, max_id)
...
func _display_pattern_editor(pattern_name: String):
	...
	if not enhanced_gridmap or not enhanced_gridmap.three_by_three_patterns.has(pattern_name):
		info_label.text = "3x3 Status: Pattern not found: " + pattern_name
		return
	
	var pattern = enhanced_gridmap.three_by_three_patterns[pattern_name].duplicate()
	_validate_pattern_size(pattern)
	
	var label = Label.new()
	label.text = "Editing Pattern: " + pattern_name
	...
	if not enhanced_gridmap.mesh_library:
		var no_lib_label = Label.new()
		no_lib_label.text = "No MeshLibrary - Cannot edit"
		grid.add_child(no_lib_label)
		info_label.text = "3x3 Status: No MeshLibrary for pattern editing"
		return
	...
	for i in range(9):
		...
		option.item_selected.connect(Callable(self, "_on_pattern_item_changed").bind(pattern_name, i, option))
...
func _on_pattern_item_changed(selected_index: int, pattern_name: String, grid_index: int, option: OptionButton):
	...
	if enhanced_gridmap.three_by_three_patterns.has(pattern_name):
		...
		_validate_pattern_size(enhanced_gridmap.three_by_three_patterns[pattern_name])
		print("Updated pattern '", pattern_name, "' item ", grid_index, " to: ", item_id)
		enhanced_gridmap.validate_3x3_items()
		info_label.text = "3x3 Status: Updated pattern " + pattern_name
...
func _show_add_pattern_dialog():
	...
	confirm_button.pressed.connect(func():
		...
		if pattern_name != "" and not enhanced_gridmap.three_by_three_patterns.has(pattern_name):
			var default_pattern = []
			for i in range(9):
				default_pattern.append(-1)  # Default to empty (-1)
			_validate_pattern_size(default_pattern)
			...
			for i in range(pattern_selector.get_item_count()):
				if pattern_selector.get_item_text(i) == pattern_name:
					...
					_display_pattern_editor(pattern_name)
					break
			info_label.text = "3x3 Status: Created pattern " + pattern_name
		else:
			info_label.text = "3x3 Status: Pattern name invalid or exists"
		dialog.queue_free()
	)
...
func _on_center_changed(value):
	var center = Vector2i(center_x_spin.value, center_z_spin.value)
	if enhanced_gridmap and not enhanced_gridmap.is_valid_3x3_placement(center, floor_spin.value):
		info_label.text = "3x3 Status: Invalid center position: " + str(center)
	else:
		info_label.text = "3x3 Status: Center set to " + str(center)

func _on_place_pattern_pressed():
	if not enhanced_gridmap or pattern_selector.get_selected() == -1:
		info_label.text = "3x3 Status: No pattern selected or no EnhancedGridMap"
		return
	
	var pattern_name = pattern_selector.get_item_text(pattern_selector.get_selected())
	var center = Vector2i(center_x_spin.value, center_z_spin.value)
	var current_floor = floor_spin.value as int
	
	if enhanced_gridmap.is_valid_3x3_placement(center, current_floor):
		enhanced_gridmap.place_3x3_structure(center, pattern_name, current_floor)
		print("Placed pattern '", pattern_name, "' at ", center)
		update_ui()
		info_label.text = "3x3 Status: Placed pattern " + pattern_name + " at " + str(center)
	else:
		info_label.text = "3x3 Status: Invalid placement position: " + str(center)
...
func _on_create_3x3_pressed():
	...
	var center = Vector2i(center_x_spin.value, center_z_spin.value)
	var current_floor = floor_spin.value as int
	var pattern_name = "default" if enhanced_gridmap.three_by_three_patterns.has("default") else enhanced_gridmap.three_by_three_patterns.keys()[0] if not enhanced_gridmap.three_by_three_patterns.is_empty() else ""
	
	if pattern_name == "":
		if not enhanced_gridmap.three_by_three_items.is_empty():
			var default_pattern = enhanced_gridmap.three_by_three_items.duplicate()
			_validate_pattern_size(default_pattern)
			enhanced_gridmap.three_by_three_patterns["default"] = default_pattern
			pattern_name = "default"
		else:
			info_label.text = "3x3 Status: No patterns or default items available"
			return
	
	if enhanced_gridmap.is_valid_3x3_placement(center, current_floor):
		enhanced_gridmap.place_3x3_structure(center, pattern_name, current_floor)
		print("Created 3x3 structure with pattern '", pattern_name, "' at ", center)
		update_ui()
		info_label.text = "3x3 Status: Created 3x3 structure at " + str(center)
	else:
		info_label.text = "3x3 Status: Invalid placement position: " + str(center)
...
func _on_clear_3x3_pressed():
	...
	if enhanced_gridmap.is_valid_3x3_placement(center, current_floor):
		_clear_3x3_structure(center, current_floor)
		update_ui()
		info_label.text = "3x3 Status: Cleared 3x3 structure at " + str(center)
	else:
		info_label.text = "3x3 Status: Invalid position for clearing: " + str(center)
...
func _on_detect_3x3_pressed():
	...
	var current_floor = floor_spin.value as int
	enhanced_gridmap.detect_existing_3x3_structures(current_floor)
	update_ui()
	info_label.text = "3x3 Status: Detected " + str(enhanced_gridmap.three_by_three_centers.size()) + " 3x3 structures"

func _clear_3x3_structure(center: Vector2i, floor: int):
	for z_offset in range(-1, 2):
		for x_offset in range(-1, 2):
			var pos = Vector3i(center.x + x_offset, floor, center.y + z_offset)
			enhanced_gridmap.set_cell_item(pos, -1)
	enhanced_gridmap.three_by_three_centers.erase(center)
	enhanced_gridmap.initialize_astar()
